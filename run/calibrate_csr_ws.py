...
from gym_ras.tool.kdl_tool import T2Frame, Frame2T
...
arm1 = ArmProxy(node_, "psa3")

while not arm1.is_connected:
    arm1.measured_cp()
# To check if the arm is connected
arm1.read_rtrk_arm_state()
print("connection: ", arm1.is_connected)

in_device = Keyboard()

def get_pose():
    cmd = in_device.get_char()
    # print("pos:",client.T_g_w_msr.p)
    return T2Frame(CsrFrame2T(arm1.measured_cp()))
...
theta += np.deg2rad(90) # align with simulation setting
