...
def train(origin_env, config, success_id=5.0, max_eps_length=300):
    ...
    if prefill_total:
        ...
        for k, v in prefill_config.items():
            ...
            print(f'Prefill {k} {to_prefill} steps.')
            if to_prefill <= 0:
                continue
    ...
    train_agent(next(train_dataset), bc_func(train_dataset))
    ...
    def train_step(tran, worker):
        if should_train(step):
            for _ in range(config.train_steps):
                ...
                mets = train_agent(next(train_dataset), bc_func(train_dataset))
    ...
    # clear add_step hook
    train_driver._on_steps = []
    train_driver._on_resets = []
    train_driver._on_episodes = []
    train_driver.on_episode(lambda ep: per_episode(ep, mode='train'))
    train_driver.on_step(lambda tran, worker: step.increment())
